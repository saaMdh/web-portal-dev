<template>
  <div flat class="o-editor-queries">
    <v-toolbar>
      <v-toolbar-title>
        {{ $t("editorHome.queriesManagement.title") }}
      </v-toolbar-title>
    ...
    <v-window v-model="step">
      <v-window-item :value="1">
        <query-list :type="'ANNOUNCEMENTS'" @editQuery="edit"></query-list>
      </v-window-item>
...
import QueriesManagement from "@/views/SiteBuilder/EditorSheet/QueriesManagement/Editor";
...
export default {
  ...
  methods: {
    ...
    async save() {
      if (this.index !== null)
        await this.editQuery({ query: this.query, index: this.index });
      else await this.addQuery({ query: this.query, type: "ANNOUNCEMENTS" });
      this.step = 1;
    },
    edit({ data, index }) {
      this.index = index;
